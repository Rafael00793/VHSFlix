        ...
        <div className="flex items-center gap-1.5 xs:gap-3 sm:gap-5">
          ...
          <div className="relative flex items-center" ref={notificationsRef}>
            <button
              ...
              className="text-zinc-400 hover:text-white hover:scale-105 active:scale-95 relative p-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 focus-visible:outline-none rounded-full transition-all cursor-pointer animate-pulse-slow"
              id="navbar-notifications-btn"
              title="Notificações de Lançamentos"
            >
              <Bell className="w-5 h-5" />
              {unreadCount > 0 && (
                <span className="absolute top-1.5 right-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-rose-600 text-[8.5px] font-bold text-white font-mono border border-zinc-950 animate-bounce">
                  {unreadCount}
                </span>
              )}
            </button>
            ...
            <AnimatePresence>
              {showNotificationsDropdown && (
                <motion.div
                  ...
                  className="absolute right-0 mt-14 w-[calc(100vw-1.5rem)] xs:w-80 sm:w-96 bg-zinc-950/98 border border-zinc-800/80 rounded-xl shadow-2xl shadow-black/80 z-[9999] overflow-hidden font-sans backdrop-blur-md"
                >
                  {/* Header do Dropdown */}
                  <div className="p-4 border-b border-zinc-900 bg-zinc-900/50 flex justify-between items-center select-none">
                    <div className="flex items-center gap-2">
                      <div className="h-6 w-1 bg-rose-600 rounded-full" />
                      <Bell className="w-4 h-4 text-rose-500" />
                      <span className="font-bold font-display text-xs text-white uppercase tracking-wider">Notificações</span>
                    </div>
                    {unreadCount > 0 && (
                      <button
                        onClick={(e) => {
                        ...
                        className="text-[10px] text-rose-500 hover:text-rose-450 font-bold uppercase tracking-wider transition-colors cursor-pointer"
                      >
                        Limpar Novas
                      </button>
                    )}
                  ...
                  <div className="max-h-[360px] overflow-y-auto divide-y divide-zinc-900/50 custom-scrollbar">
                    {notifications.length === 0 ? (
                      <div className="p-10 text-center text-zinc-500 flex flex-col items-center justify-center gap-3 select-none">
                        <div className="w-12 h-12 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-700">
                          <Bell className="w-5 h-5" />
                        </div>
                        <p className="text-[10.5px] font-mono uppercase tracking-wider text-zinc-400">Tudo em Dia</p>
                        <p className="text-[10px] text-zinc-650 max-w-[200px] leading-relaxed">Nenhuma novidade no momento. Fique atento aos novos lançamentos!</p>
                      </div>
                    ) : (
                      notifications.map(notif => {
                        const matchedMovie = movies.find(m => m.id === notif.movieId);
                        const posterUrlToUse = notif.posterUrl || matchedMovie?.posterUrl;
                        
                        // Relative time calculation
                        let statusText = '';
                        try {
                          const now = new Date();
                          const past = new Date(notif.createdAt);
                          const diffMs = now.getTime() - past.getTime();
                          if (!isNaN(diffMs)) {
                            const diffSec = Math.floor(diffMs / 1000);
                            if (diffSec < 60) statusText = 'Agora';
                            else {
                              const diffMin = Math.floor(diffSec / 60);
                              if (diffMin < 60) statusText = `Há ${diffMin} min`;
                              else {
                                const diffHr = Math.floor(diffMin / 60);
                                if (diffHr < 24) statusText = `Há ${diffHr} h`;
                                else {
                                  const diffDays = Math.floor(diffHr / 24);
                                  if (diffDays === 1) statusText = 'Ontem';
                                  else if (diffDays < 7) statusText = `Há ${diffDays} dias`;
                                  else statusText = past.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' });
                                }
                              }
                            }
                          }
                        ...
                        return (
                          <div
                            ...
                            className={`p-3 hover:bg-zinc-900/90 transition-all flex gap-3.5 cursor-pointer relative ${!notif.isRead ? 'bg-zinc-900/35 border-l-2 border-l-rose-600' : ''}`}
                          >
                            {/* Poster cover art on the left side - Netflix style */}
                            {posterUrlToUse ? (
                              <div className="w-12 h-18 rounded overflow-hidden flex-shrink-0 border border-zinc-800 shadow-md">
                                <img
                                  src={posterUrlToUse}
                                  alt=""
                              ...
                              <div className="w-12 h-18 rounded bg-gradient-to-b from-zinc-900 to-black border border-zinc-800 flex flex-col items-center justify-center flex-shrink-0 text-zinc-600">
                                <span className="text-[10px] font-mono leading-none font-bold text-center">VHS</span>
                                <span className="text-[8px] opacity-40 uppercase tracking-widest">{notif.type === 'series' ? 'TV' : 'FM'}</span>
                              </div>
                            )}
                            ...
                            <div className="flex-1 min-w-0 flex flex-col justify-between">
                              <div className="space-y-0.5">
                                <div className="flex justify-between items-center gap-2">
                                  <h4 className="text-[11px] font-extrabold text-white leading-snug tracking-tight truncate max-w-[170px] uppercase">
                                    {notif.title}
                                  </h4>
                                  <span className="text-[8px] text-zinc-500 font-mono whitespace-nowrap">{statusText}</span>
                                </div>
                                <p className="text-[11px] text-zinc-400 leading-snug font-normal line-clamp-2">
                                  {notif.message}
                                </p>
                              </div>
                              ...
                              <div className="flex items-center gap-1.5 mt-1">
                                {matchedMovie && (
                                  <span className="text-[8px] bg-zinc-900/60 text-zinc-400 font-bold border border-zinc-800 px-1.5 py-0.5 rounded font-mono truncate max-w-[120px]">
                                    {matchedMovie.category}
                                  </span>
                                )}
                                {!matchedMovie && (
                                  <span className="text-[8px] bg-zinc-900/60 text-zinc-400 font-bold border border-zinc-800 px-1.5 py-0.5 rounded font-mono uppercase">
                                    {notif.type === 'series' ? 'Série' : notif.type === 'movie' ? 'Filme' : 'Sistema'}
                                  </span>
                                )}
                                <span className="text-[9px] text-rose-500 hover:text-rose-450 font-extrabold font-mono tracking-wider ml-auto flex items-center gap-0.5 transition-colors">
                                  {notif.movieId ? 'Assistir' : 'Ver'} ➔
                                </span>
                              </div>
                            </div>
                          </div>
                  ...
                  <div className="p-2.5 border-t border-zinc-900 bg-zinc-950 text-center select-none">
                    <span className="text-[8px] uppercase font-mono tracking-widest text-zinc-650 font-bold">Lançamento Exclusivo VHSFLIX</span>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
